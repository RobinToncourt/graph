...
fn get_index<L>(labels: &[L], label: &L) -> anyhow::Result<usize>
where
    L: PartialEq,
{
...
mod test_graph {
    ...
    fn create_graph_wiki() -> Graph<String, usize> {
        let labels: Vec<String> = vec!["1", "2", "3", "4", "5", "6"]
            .into_iter()
            .map(str::to_string)
            .collect();
    ...
    fn create_graph_wiki_2() -> Graph<String, usize> {
        let labels: Vec<String> = vec!["A", "B", "C", "D", "E", "F"]
            .into_iter()
            .map(str::to_string)
            .collect();
    ...
    fn test_get_index() {
        ...
        let err: &str = get_index(&labels, &"7")
            .unwrap_err()
            .downcast::<&str>()
            .unwrap();
    ...
    fn test_get_connected_nodes() {
        let graph: Graph<String, usize> = create_graph_wiki();
        let expected: Vec<String> = vec!["2", "3", "6"]
            .into_iter()
            .map(str::to_string)
            .collect();
    ...
    fn test_get_connected_nodes_and_path() {
        ...
        let six = "6".to_string();
        let expected: Vec<(&String, &usize)> = vec![(&deux, &7), (&trois, &9), (&six, &14)];

        assert_eq!(
            graph.get_connected_nodes_and_path(&s!("1")).unwrap(),
            expected
        );
    }
}
